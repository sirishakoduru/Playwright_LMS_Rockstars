...
Given('Admin is on login Page in LMS for Logout', async function() {
    const commonPage = this.poManager.getCommonPage()
    await commonPage.navigateToLoginPage()
    
  });
  
  When('Admin enter valid data in all field and  {string} clicks login button for logout', async function() {
    const commonPage = this.poManager.getCommonPage()
